/**
 * 期末考试 编程题＃5：走出迷宫（Use dynamic memory, Time Limit Exceeded）
 *
...
int takeThisWay(char **maze, int n, int m, int steps);
bool canMoveOneStep(char **maze, int n, int m, char direction);
position getPosition(char **maze, int n, int m, char c);
void releaseMemory(char **p, int n, int m);
...
int takeThisWay(char **maze, int n, int m, int steps)
{
    ...
    if (steps == -1) {
        ...
        position t = getPosition(maze, n, m, 'T');
