...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
    ...
    if (apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
        ...
        check(rv == 0, "Faield to process the DEPENDS: %s", url);

        // this indicates that nothing nees to be done
        return 0;
    
    } else if (apr_fnmatch(TAR_GZ_PAT, info.path, 0) == APR_SUCCESS) {
        if (info.scheme) {
            rc = Shell_exec(CURL_SH, "URL", url, "TARGET", TAR_GZ_SRC, NULL);
            check(rc == 0, "Failed to curl sources: %s", url);
        }
    ...
    // indicates that an install nees to actually run
    return 1;
error:
    return -1;
}
        ...
        const char *make_opts, const char *install_opts)
{
    ...
    check(access(BUILD_DIR, X_OK | R_OK | W_OK) == 0,
            "Build directory dosen't exist: %s", BUILD_DIR);
        ...
        const char *make_opts, const char *install_opts)
{
    ...
    if (rc == 1) {
        log_info("Package %s already isntalled.", url);
        return 0;
    }
