...
\begin{algorithm}[htb]
    \Repeat{iteration = MaxIteration}{
        ...
        $\Matrix{Z^{[1]}} = \Matrix{W^{[1]}}\Matrix{X}$ \\
...
Because $z^{[2]} = \Matrix{W^{[2]}}\Vector{a^{[1]}} + \Vector{b^{[2]}}$, once we get
$\text{d}{z^{[2]}}$, we can continue to compute $\text{d}\Matrix{W^{[2]}}$ and $\text{d}{b^{[2]}}$:
\begin{IEEEeqnarray*}{rCl}
